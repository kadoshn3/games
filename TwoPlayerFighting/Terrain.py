...
class terrain():
    def __init__(self):
        self.env_images = ['images\locker_room.jpg', 'images\menu\selection_screen.jpg', 'images\Backyard.PNG', 'images\Bamboo_house.jpg', 'images\super_smash_bros.png', 'images\jungle.jfif']
        self.env_select = 2
        self.image = PIL.Image.open(self.env_images[self.env_select])
        self.env_width, self.env_height = self.image.size
        self.screen_shift = int(0*self.env_width)#int(0.5*self.env_width)
